...
np.save("../temp_data/shap_values_zonal_speed", shap_values)
...
sc = ax1.scatter(np.abs(X[fname].values), np.abs(shap_values[:,ix]), c=X[iname].values, cmap=plt.cm.coolwarm, s=5, vmin=1, vmax=8)
ax1.set_ylabel("Shapley value (km hr$^{-1}$)")
ax1.set_xlabel("Mean 850 hPa zonal wind (m s$^{-1}$)")
ax1.set_title("(a)")
...
iname = 'vortex_depth'
ix = np.where(feature_names==fname)[0][0]
sc = ax2.scatter(X[fname].values, df.zonal_speed.values, c=X[iname].values, cmap=plt.cm.jet, s=5, vmax=1.5)
ax2.set_ylabel("Shapley value (km hr$^{-1}$)")
ax2.set_xlabel("Mean 500 hPa zonal wind (m s$^{-1}$)")
ax2.set_title("(b)")
...
cb1.set_label("Vortex depth (unitless)")
'''
